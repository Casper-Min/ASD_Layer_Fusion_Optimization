...
def res_parse(schedule_info_list, resource, cost_model, sg, network,
        ...
        for section_info in schedule_info_list:
            ...
            for layer, point in zip(fusion_group, point_list):
                if point:
                    ...
                    utilization_p += xx * yy * network[layer].total_ops / resource.para_count_list[0]
                    # total_ops += network[layer].total_ops * 1.15
                    total_ops += network[layer].total_ops
                    total_ops_p += network[layer].total_ops
            if total_ops_p:
                u = utilization_p / total_ops_p
                alu += total_ops_p / u
    ...
    costs[4] = 0.56*alu
    for i in range(5):
        costs2[i] = round(costs[i]) / alu
    for i in range(5):
        costs[i] = round(costs[i]) / 1e10

    if is_print:
        print('total Ops / DRAM access(KB):', total_ops , '/', off_chip_overall * resource.precision / 8 / 1024)
        print('DRAM access breakdown[ifmap, ofmap, filter](KB):',[ifmap * resource.precision / 8 / 1024, ofmap * resource.precision / 8 / 1024, filter * resource.precision / 8 / 1024])
        print('DRAM access/Ops (Btye/Ops):', (off_chip_overall * resource.precision / 8) / total_ops , '\n')

        print('total energy(1e10 pJ):', round(total_cost) / 1e10)
...
def res_parse_timeloop(schedule_info_list, resource, cost_model, sg, network):
